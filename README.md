...
- 屏幕录制权限改用 ScreenCaptureKit 实际探测；即使 Core Graphics 预检结果过期，也会继续尝试真实系统音频捕获。
- API Key / Token 优先保存在 macOS 钥匙串；系统钥匙串不可用时回退到仅当前用户可读的本机保护文件，不写入会议文件或日志。
- Token 保存后立即回读验证，设置页明确显示实际保存结果，关闭并重新打开应用后自动载入。

...
启用 ASR 后，临时 WAV 短片段会发送到所选服务，识别完成后从临时目录删除。多次重试仍失败或停止时未完成的片段与处理清单保存在 `~/Library/Application Support/MinuteFlow/PendingTranscriptions/`；v0.6.0 暂不提供自动重新提交入口，原始 M4A 始终保留。启用总结时，逐字稿文本会发送到同一服务连接中的总结模型。
...
测试覆盖权限无副作用读取、音量计算、WAV 转换、双路协调与运行中降级、异常退出恢复、跨会议总结竞态与长文本分段总结、会议持久化与删除、Token 显式保存/加密回退/跨启动、Token Plan 接口解析和逐字稿规范化。

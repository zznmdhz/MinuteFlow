...
final class MeetingRepositoryTests: XCTestCase {
    ...
    func testCreatesSeparateAudioDestinationsAndPersistsMetadata() throws {
        ...
        XCTAssertTrue(session.systemAudioURL?.lastPathComponent.hasSuffix("_系统声音.m4a") == true)
        XCTAssertTrue(session.microphoneAudioURL?.lastPathComponent.hasSuffix("_麦克风.m4a") == true)
        ...
        XCTAssertEqual(documentURL.lastPathComponent, "AI排版文稿.md")
        XCTAssertEqual(try repository.loadFormattedDocument(sessionID: session.id), "# 排版文稿")
        XCTAssertEqual(repository.transcriptMarkdownURL(for: session.id).lastPathComponent, "逐字稿.md")
        XCTAssertEqual(repository.summaryMarkdownURL(for: session.id).lastPathComponent, "会议纪要.md")

        session = try repository.renameSession(session, to: "季度经营复盘")
        XCTAssertEqual(session.title, "季度经营复盘")
        XCTAssertEqual(repository.sessionDirectory(for: session.id).lastPathComponent, "季度经营复盘")
        XCTAssertTrue(FileManager.default.fileExists(atPath: repository.transcriptMarkdownURL(for: session.id).path))

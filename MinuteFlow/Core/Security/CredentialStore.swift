...
enum CredentialStore {
    static func read(
        ...
        // A protected-file credential only exists after the user explicitly
        // chose that fallback. It must therefore be authoritative. Older
        // builds preferred any stale Keychain item, which made a newly tested
        // local credential appear to "expire" after the next launch.
    ...
    private static func writeProtectedFile(_ value: String, to url: URL, directory: URL) throws {
        ...
        // AES-GCM plus mode 0600 is the actual macOS protection here.
        // `.completeFileProtection` is an iOS-style option and can make atomic
        // replacement fail intermittently on macOS temporary/container volumes.

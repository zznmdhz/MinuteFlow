...
final class RemoteRealtimeTranscriptionService: @unchecked Sendable {
    ...
    /// A transport hard cut may continue one utterance, but a display paragraph
    /// is never allowed to grow for an entire meeting when VAD is uncertain.
    private let maximumParagraphDuration: TimeInterval = 40
    ...
    func start(
        ...
        try queue.sync {
            ...
            for source in sources {
                accumulators[source] = try ChunkAccumulator(
                    sessionID: sessionID,
                    source: source,
                    directory: directory,
                    timelineOffset: max(0, timelineOffset)
                )
            }
            running = true
        }
        if timelineOffset > 0 {
            onStatus?("已从 \(Int(timelineOffset)) 秒热启动转写 · 停顿后定稿")
        } else {
            onStatus?("智能分段已启动 · 停顿后定稿")
        }
    }

    /// Immediately abandons the old runtime configuration so a newly saved
    /// credential can take effect without stopping the recording.
    func abortForReconfiguration() {
        queue.sync {
            running = false
            currentGeneration = nil
            cancelOutstandingPreviews()
            pendingUploads.forEach { job in
                if job.kind == .preview {
                    try? FileManager.default.removeItem(at: job.url)
                } else {
                    preserveFailedChunk(job, reason: "模型配置已更新，可使用事后转写补齐")
                }
            }
            pendingUploads.removeAll()
            activeUploads.values.forEach { $0.cancel() }
            activeUploads.removeAll()
            activeUploadKinds.removeAll()
            for accumulator in accumulators.values {
                accumulator.removeUnusedWriters()
            }
            accumulators.removeAll()
            recognitionGroups.removeAll()
            finalizedGroupIDs.removeAll()
        }
    }
        ...
        queue.async { [weak self] in
            ...
            do {
                ...
                if shouldFlushForSilence {
                    ...
                    let closesParagraph = accumulator.groupDuration >= self.maximumParagraphDuration
                        || accumulator.trailingSilence >= 0.25
                    let reason: TranscriptBoundaryReason = accumulator.trailingSilence >= 0.25
                        ? .lowEnergyHardLimit
                        : .forcedHardLimit
                    try self.flush(accumulator, boundaryReason: reason, isTerminal: closesParagraph)
...
private final class ChunkAccumulator: @unchecked Sendable {
    ...
    var groupStartTime: TimeInterval = 0
    var hasSpeech = false
    ...
    var groupDuration: TimeInterval {
        max(0, writerStartTime + writer.duration - groupStartTime)
    }
    ...
    init(
        ...
        writerStartTime = timelineOffset
        groupStartTime = timelineOffset
    }
    ...
    func observe(level: Float, duration: TimeInterval) {
        ...
        if observation.speechStarted, !hasSpeech {
            hasSpeech = true
            // Capture callbacks are normally short, but tests, imports and
            // some aggregate devices can deliver multi-second buffers. The
            // whole observed buffer may contain the speech that opened VAD.
            speechStartedAtWriterDuration = max(0, writer.duration - duration)
        }
    }
    ...
    func advance(
        ...
        voiceActivity.reset(continuingSpeech: continuingGroup)
        if continuingGroup {
            sequence += 1
        } else {
    ...
    func discardAndAdvance(
        ...
        groupStartTime = writerStartTime
        voiceActivity.reset(continuingSpeech: false)
        overlapBefore = 0
        previewSubmitted = false
    }

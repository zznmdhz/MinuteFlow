...
private struct SidebarView: View {
    ...
    var body: some View {
        ...
        .alert(
            ...
            Text("会议标题和本机文件夹会同步重命名；录音、逐字稿和纪要仍保持关联。")
        }
    }
}
                ...
                HStack(spacing: 9) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text("\(session.startTime.formatted(date: .abbreviated, time: .shortened)) · \(DurationFormatter.string(from: session.duration))")
                            .font(.caption2)
                            .foregroundStyle(.secondary)
                        Label(session.location?.displayName ?? "地点未记录", systemImage: "location.fill")
                            .font(.caption2)
                            .foregroundStyle(.tertiary)
                            .lineLimit(1)
                    }
...
private struct RecordingFilesPanel: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if let silentSources = session.silentSources, !silentSources.isEmpty {
                Label(
                    "\(silentSources.map(\.title).joined(separator: "、"))没有检测到有效声音",
...
private struct TranscriptPanel: View {
    ...
    @State private var speakerPendingRename: String?
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack {
                ...
                if coordinator.currentSession != nil,
                   coordinator.transcriptSegments.contains(where: \.isFinal) {
                    if coordinator.isAnalyzingSpeakers {
                        ProgressView().controlSize(.small)
                    } else {
                        Button(
                            coordinator.speakerTurns.isEmpty ? "分析说话人" : "重新分析",
                            systemImage: "person.2.wave.2"
                        ) {
                            Task { await coordinator.analyzeCurrentMeetingSpeakers() }
                        }
                        .controlSize(.small)
                        .disabled(coordinator.status.isActive)
                        .help("在本机分析声纹并生成说话人时间轴，不上传声纹")
                    }
                ...
                if coordinator.shouldOfferPostTranscription,
                   !coordinator.transcriptSegments.isEmpty {
                    if coordinator.isPostTranscribing {
                        Button("取消事后转写", role: .cancel) {
                            coordinator.cancelPostRecordingTranscription()
                        }
                        .controlSize(.small)
                    } else if coordinator.modelSettings.asrIsConfigured {
                        Button(coordinator.postTranscriptionButtonTitle, systemImage: "waveform.badge.plus") {
                            coordinator.startPostRecordingTranscription()
                        }
                        .controlSize(.small)
                    }
                }
                HStack(spacing: 6) {
                    if coordinator.status.isActive || coordinator.isPostTranscribing {
                        ProgressView().controlSize(.small)
                    }
            ...
            if coordinator.isPostTranscribing {
                VStack(alignment: .leading, spacing: 5) {
                    ProgressView(value: coordinator.postTranscriptionProgress)
                    Text(coordinator.postTranscriptionMessage)
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
            ...
            if !coordinator.transcriptSegments.isEmpty {
                ...
                Text(coordinator.speakerAnalysisMessage)
                    .font(.caption)
                    .foregroundStyle(coordinator.speakerTurns.isEmpty ? Color.secondary : Color.green)
            }
            ...
            if coordinator.transcriptSegments.isEmpty {
                VStack(spacing: 9) {
                    ...
                    if !coordinator.modelSettings.asrIsConfigured {
                        Button("配置 AI 语音识别") { openSettings() }
                            .buttonStyle(.link)
                    } else if coordinator.isPostTranscribing {
                        Button("取消事后转写") {
                            coordinator.cancelPostRecordingTranscription()
                        }
                        .buttonStyle(.bordered)
                    } else if coordinator.shouldOfferPostTranscription {
                        Button(coordinator.postTranscriptionButtonTitle) {
                            coordinator.startPostRecordingTranscription()
                        }
                        .buttonStyle(.borderedProminent)
                    }
                            ...
                            VStack(alignment: .leading, spacing: 3) {
                                HStack(spacing: 5) {
                                    if let speakerID = segment.speakerID {
                                        Button(coordinator.speakerDisplayName(for: speakerID)) {
                                            speakerPendingRename = speakerID
                                            speakerNameDraft = coordinator.speakerDisplayName(for: speakerID)
                                        }
                                        .buttonStyle(.plain)
                                        .font(.caption2.weight(.semibold))
                ...
                .background(Color(nsColor: .textBackgroundColor).opacity(0.18), in: RoundedRectangle(cornerRadius: 10))
            }
        }
        .padding(20)
        .background(Color(nsColor: .controlBackgroundColor).opacity(0.72), in: RoundedRectangle(cornerRadius: 16))
        .alert(
            "重命名说话人",
            isPresented: Binding(
                get: { speakerPendingRename != nil },
                set: { if !$0 { speakerPendingRename = nil } }
            )
        ) {
            TextField("姓名或角色", text: $speakerNameDraft)
            Button("保存") {
                if let speakerPendingRename {
                    coordinator.renameSpeaker(speakerPendingRename, to: speakerNameDraft)
                }
                speakerPendingRename = nil
            }
            .disabled(speakerNameDraft.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
            Button("取消", role: .cancel) { speakerPendingRename = nil }
        } message: {
            Text("名称只保存在这条会议中，例如“王总”“产品经理”或“客户”。")
        }
    }
    ...
    private func speakerColor(_ speakerID: String) -> Color {
        let number = Int(speakerID.split(separator: "-").last ?? "1") ?? 1
        let colors: [Color] = [.blue, .orange, .purple, .green, .pink, .teal]
        return colors[(max(1, number) - 1) % colors.count]
    }
